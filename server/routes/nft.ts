...
function getNftContract() {
    return new web3.eth.Contract(NFTAbi as AbiItem[], process.env.LETMEDOITFORYOU_CA);
}
...
router.post('/sellData', async (req, res) => {
    try {
        ...
        const LMDIFYdeployed = new web3.eth.Contract(NFTAbi as AbiItem[], process.env.LETMEDOITFORYOU_CA);
        const APPID = LMDIFYdeployed.methods.setApprovalForAll(process.env.BuySell_CA, true).encodeABI();
        const objAproval: {
            nonce: number;
            from: string;
            to: string | undefined;
        ...
        const deployed = new web3.eth.Contract(BuySellAbi as AbiItem[], process.env.BuySell_CA);
        const bigNumberValue = BigNumber.from(Math.floor(req.body.priceValue * 10 ** 18).toString());
        const SellData = deployed.methods.SalesToken(req.body.hash, bigNumberValue).encodeABI();
        const BuyObj: {
            nonce: number;
            from: string;
            to: string | undefined;
        ...
        await db.NFT.update({ price: req.body.priceValue }, { where: { hash: req.body.hash } });
...
router.post('/render', async (req, res) => {
    await db.NFT.update({ price: 0, owner: req.body.account }, { where: { hash: req.body.data } });
});
